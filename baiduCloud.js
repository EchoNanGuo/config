...
(function() {
    ...
    obj.initPlayer = function (player) {
        ...
        obj.playerReady(player, function(player) {
            ...
            obj.isAppreciation(function (data) {
                data ? player.on("contextmenu_show", function () {
    ...
    obj.dPlayerSetting = function (player) {
        ...
        $(".dplayer-setting-autoposition").on("click", function() {
            ...
            obj.isAppreciation(function (data) {
                data ? obj.showDialog() : (toggle.get(0).checked = checked, user.set("autoposition", Number(checked)));
            });
        });
        $(".dplayer-setting-autoplaynext").on("click", function() {
            ...
            var checked = !toggle.is(":checked");
            obj.isAppreciation(function (data) {
                data ? obj.showDialog() : (toggle.get(0).checked = checked, user.set("autoplaynext", Number(checked)));
            });
        });
        $(".dplayer-setting-soundenhancement").on("click", function() {
            ...
            var checked = !toggle.is(":checked");
            obj.isAppreciation(function (data) {
                data ? obj.showDialog() : (toggle.get(0).checked = checked, user.set("soundenhancement", Number(checked)), obj.dPlayerSoundEnhancer(player));
            });
        });
        $(".dplayer-setting-imageenhancement").on("click", function() {
            ...
            var checked = !toggle.is(":checked");
            obj.isAppreciation(function (data) {
                data ? obj.showDialog() : (toggle.get(0).checked = checked, user.set("imageenhancement", Number(checked)), obj.dPlayerImageEnhancer(player));
            });
        });
        player.on("playing", () => {
            if (!player.joySound) {
    ...
    obj.dPlayerCustomSpeed = function (player) {
        ...
        $(".dplayer-setting-speed-item[data-speed='自定义']").on("click", function() {
            if (document.querySelector(".dplayer .dplayer-menu").classList.contains("dplayer-menu-show")) {
                ...
                obj.isAppreciation(function (data) {
                    data ? (obj.msg("\u8bf7\u4f7f\u7528\u7231\u53d1\u7535\u4f53\u9a8c\u6d4b\u8bd5\u529f\u80fd"), obj.showDialog()) : custombox.css("display") == "block" ? (custombox.css("display", "none"), player.setting.hide()) : custombox.css("display", "block");
                });
            }
    ...
    obj.appreciation = function (player) {
        Date.now() - (GM_getValue("appreciation_show") || 0) > 86400000 && setTimeout(() => {
            obj.isAppreciation(function (data) {
                data ? (alert("\u672c\u811a\u672c\u672a\u5728\u4efb\u4f55\u5e73\u53f0\u51fa\u552e\u8fc7\u0020\u5982\u679c\u89c9\u5f97\u559c\u6b22\u591a\u8c22\u60a8\u7684\u8d5e\u8d4f"), player.contextmenu.show(player.container.offsetWidth / 2.5, player.container.offsetHeight / 3)) : data.notice && alert(data.notice);
            });
